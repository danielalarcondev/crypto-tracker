...
const ASSETS_ENDPOINT = 'https://api.coincap.io/v2/assets';
const DEFAULT_ASSETS_LIMIT = 100;
const DEFAULT_ASSETS_OFFSET = 0;

const DEFAULT_ERROR_CODE = 500;
const DEFAULT_ERROR_MESSAGE = 'Something went wrong';

const prepareUrl = (params: GetAssetsPayload): string => {
...
export const getAssets = async ({ search, ids, limit, offset }: GetAssetsPayload): Promise<GetAssetsResponse> => {
    try {
        const url = prepareUrl({ search, ids, limit, offset });
